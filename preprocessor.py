...
def filter_reviews_by_length(df_reviews_all):
    ...
    """
    df_reviews_all['review_text']=df_reviews_all['review_text'].map(lambda review: review[14:]) #remove date, which is at start of every review string (for now works for May only)
    df_reviews_all['review_length'] = df_reviews_all['review_text'].map(lambda review: len(review))
    len_mean = df_reviews_all['review_length'].mean()
    standard_deviation = df_reviews_all['review_length'].std()
    df_reviews_all = df_reviews_all[(df_reviews_all.review_length > 4 )& (df_reviews_all.review_length <= len_mean + standard_deviation)]
...
def filter_non_ascii(review_text):
    ...
    """
    ascii_pattern = re.compile(r'^[\s~`!@#$%^&*()_+=\[\]\\{}|;\':\",.\/<>?a-zA-Z0-9-]+$',re.ASCII)    
...
if __name__ == '__main__':
    ...
    df_reviews_all = filter_reviews_by_length(df_reviews_all)   #remove too long reviews
    df_reviews_all['review_text'] = df_reviews_all['review_text'].apply(filter_emojis)  #remove emojis
    df_reviews_all['review_text'] = df_reviews_all['review_text'].apply(filter_non_ascii)   #remove non ascii reviews 
    df_reviews_all['review_text']= df_reviews_all['review_text'].apply(remove_punctuation) # remove punctuation
    df_reviews_all = df_reviews_all[(df_reviews_all.review_text != "")] #remove empty strings
